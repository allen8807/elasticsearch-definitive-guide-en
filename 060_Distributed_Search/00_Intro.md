...
basic _create-read-update-delete_ (CRUD) requests<!--((("CRUD (create-read-update-delete) operations")))--> that we discussed in [distributed-docs].
...
`_index`, `_type`, and `routing-value` (which defaults to the
